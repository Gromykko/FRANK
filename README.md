...
Location availability is independent. If a known location has no compatible forecast yet and a provider returns an explicitly classified transient failure (network timeout, 429, or 5xx), the Worker returns a versioned `FORECAST_INITIALIZING` 503 with `Retry-After`; it never exposes the provider's response. A ten-minute, expiring KV cooldown reduces repeat request waves from browsers and public `?warm=1` calls, while the trusted cron retries on its normal schedule. KV is eventually consistent rather than a global lock, so exact cross-region single-flight would require a Durable Object; the sequential deployment warm normally keeps visitors from becoming the first builders. Invalid provider shapes, unsupported 4xx responses, code errors, deadlines, and storage failures stay hard failures and can never be relabeled as initialization. `/health` deliberately remains red while any location is missing or stale, so monitoring stays honest even when a release is allowed to serve the other locations.

## Privacy

FRANK has no accounts, forms, device-location request, advertising, cookies, or third-party product-analytics service. The selected area, language, theme, per-location safety limits, and last usable forecast are stored in the browser only; a scoped control in the footer deletes those FRANK-owned values without clearing other projects on the shared `github.io` origin. Browser forecast copies and Worker KV forecast keys are isolated by payload version so an old open tab and a new release cannot overwrite each other's rollback/offline copy.

The browser sends only the requested location id and ordinary HTTP metadata to the forecast Worker—there is no settings/profile request body and the redundant timestamp cache-buster has been removed. GitHub Pages and Cloudflare still process ordinary connection data such as IP address for hosting and security. Cloudflare automatic request/response invocation logs are disabled; deliberately emitted operational events and failures remain available briefly for diagnosis. A visitor-triggered cold/background provider fetch can forward visitor-IP headers to MET Norway, DMI, or MeteoAlarm together with FRANK's fixed forecast coordinate, never device GPS. The in-app bilingual technical note explains this boundary and links directly to the providers' published data sources. It is deliberately not presented as a complete legal privacy notice until the operator publishes a verified identity and private contact channel. If analytics, advertising, embedded social widgets, or unrelated client-side storage are added later, reassess both the notice and Danish cookie-consent requirements before release.
...
npm run worker:active-version # record the current rollback target
npm run worker:deploy
npm run worker:active-version # copy the newly deployed version id
npm run worker:warm -- --base-url https://frank-forecast.example.workers.dev --expected-worker-version-id <new-version-id>
```
...
The normal Pages workflow deliberately does **not** receive Cloudflare credentials. Both release workflows verify that generated binding types are current, type-check the Worker independently from the browser app, and execute the route/KV contract inside Cloudflare's local Workers runtime before bundling. Worker releases use the separate **Deploy forecast Worker** workflow, which is manual, runs only from `main`, and is attached to the protected `worker-production` GitHub environment. Configure that environment with `CLOUDFLARE_API_TOKEN` and `CLOUDFLARE_ACCOUNT_ID`; required reviewers are recommended. The job audits dependencies and runs the full lint/test suite before using the repository's pinned Wrangler. Before deployment it records the exact version currently serving 100% of production traffic. After deployment it captures the new active version and passes that immutable id to `worker:warm`. Every Worker response exposes Cloudflare's native version metadata in `X-FRANK-Worker-Version`, so the gate retries boundedly while an older or unidentified edge is still serving and never accepts a same-schema response from the wrong release. Ready locations must also match their id and payload version; a location may instead return only the exact versioned initialization contract, which is reported as an amber GitHub warning and does not block the other locations. The final health reconciliation tolerates both Worker and KV propagation delay and permits missing locations only when those same locations returned typed initialization from the expected release. Ready locations with old data but current upstream checks also report amber because rollback cannot make a provider publish fresher data; a stale check clock remains a hard failure because it can signal broken refresh scheduling. Storage faults, malformed contracts, internal failures, or any unexpected missing location fail the gate and roll back to the exact recorded version—not merely the preceding upload. The protected upgrade workflow therefore requires an existing single-version deployment; provision a brand-new Worker separately and run this gate before publishing Pages.
...
For a local/manual equivalent, record `npm run worker:active-version` before deployment as the rollback target, run `npm run worker:deploy`, capture the new id with the same active-version command, then run `npm run worker:warm -- --base-url <public-worker-url> --expected-worker-version-id <new-version-id>`. Never reverse the compatibility-upgrade order merely to make both deployments appear simultaneous: an older Worker can continue serving the compatible frontend while a newer incompatible Worker can strand the old frontend.
