...
describe('SafetyManualModal profile evidence', () => {
  it('explains the thresholds, measurement clocks, local override, and primary sources', () => {
    ...
    expect(host.textContent).toContain('Wave height is below its automatic check point.');
    expect(host.textContent).toContain('Wave height is from its automatic check point through your selected maximum.');
    expect(host.textContent).toContain('FRANK calculates the point at 80% and rounds it to the same precision as the forecast.');
    expect(host.textContent).toContain('the remaining room is not a guaranteed safety margin');
    expect(host.textContent).toContain('10-minute mean wind at 10 m');
    expect(host.textContent).toContain('Mean wind and any enabled gust check are below their automatic check points.');
    expect(host.textContent).toContain('Mean wind or an enabled gust check is from its automatic check point through its maximum.');
